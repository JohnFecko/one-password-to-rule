...
const Spinner = class {
  ...
  getName() {
    const name = this.name || core.config.get("spinner");
    ...
    return core.h(
      core.Host,
      {
        class: Object.assign({}, theme.createColorClasses(this.color), {
          [mode]: true,
          [`spinner-${name}`]: true,
          "spinner-paused": !!this.paused || core.config.getBoolean("_testing")
        })
      },
      svgs
    );
...
const buildCircle = (spinner, duration, index, total) => {
  ...
  return core.h(
    "svg",
    { viewBox: "0 0 64 64", style: data.style },
    core.h("circle", { transform: "translate(32,32)", r: data.r })
  );
};
const buildLine = (spinner, duration, index, total) => {
  ...
  return core.h(
    "svg",
    { viewBox: "0 0 64 64", style: data.style },
    core.h("line", { transform: "translate(32,32)", y1: data.y1, y2: data.y2 })
  );
};
