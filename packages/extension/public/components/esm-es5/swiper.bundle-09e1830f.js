  ...
  if (selector) {
    ...
    if (typeof selector === "string") {
      ...
      if (html_1.indexOf("<") >= 0 && html_1.indexOf(">") >= 0) {
        var toCreate = "div";
        if (html_1.indexOf("<li") === 0) toCreate = "ul";
        if (html_1.indexOf("<tr") === 0) toCreate = "tbody";
        if (html_1.indexOf("<td") === 0 || html_1.indexOf("<th") === 0)
          toCreate = "tr";
        if (html_1.indexOf("<tbody") === 0) toCreate = "table";
        if (html_1.indexOf("<option") === 0) toCreate = "select";
...
function outerWidth(includeMargins) {
  if (this.length > 0) {
    if (includeMargins) {
      ...
      return (
        this[0].offsetWidth +
        parseFloat(styles_1.getPropertyValue("margin-right")) +
        parseFloat(styles_1.getPropertyValue("margin-left"))
      );
    }
...
function outerHeight(includeMargins) {
  if (this.length > 0) {
    if (includeMargins) {
      ...
      return (
        this[0].offsetHeight +
        parseFloat(styles_2.getPropertyValue("margin-top")) +
        parseFloat(styles_2.getPropertyValue("margin-bottom"))
      );
    }
...
function nextAll(selector) {
  ...
  while (el.nextElementSibling) {
    ...
    if (selector) {
      if ($(next_1).is(selector)) nextEls.push(next_1);
    } else nextEls.push(next_1);
    el = next_1;
  }
...
function prevAll(selector) {
  ...
  while (el.previousElementSibling) {
    ...
    if (selector) {
      if ($(prev_1).is(selector)) prevEls.push(prev_1);
    } else prevEls.push(prev_1);
    el = prev_1;
  }
...
function parents(selector) {
  ...
  for (var i = 0; i < this.length; i += 1) {
    ...
    while (parent_1) {
      if (selector) {
        if ($(parent_1).is(selector)) parents.push(parent_1);
      } else {
...
function updateSlides() {
  ...
  for (var i = 0; i < slidesLength; i += 1) {
    ...
    if (slide_1.css("display") === "none") continue; // eslint-disable-line
  ...
  for (var i = 0; i < slides.length; i += 1) {
    ...
    var slideProgress =
      (offsetCenter +
        ...
        slide_2.swiperSlideOffset) /
      (slide_2.swiperSlideSize + params.spaceBetween);
...
function slideToClosest(speed, runCallbacks, internal) {
  ...
  if (snapIndex < swiper.snapGrid.length - 1) {
    var translate_1 = swiper.rtlTranslate
      ? swiper.translate
      : -swiper.translate;
...
function setBreakpoint() {
  ...
  var activeIndex = swiper.activeIndex,
    ...
    _a = swiper.loopedSlides,
    loopedSlides = _a === void 0 ? 0 : _a,
    params = swiper.params;
